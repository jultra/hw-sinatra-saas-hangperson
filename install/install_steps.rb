...
When(/^I clone "(.*)" branch of public skeleton "(.*)"$/) do |branch, skel_repo|
  ...
  if ! Dir.exists?(skel)
    ...
    run_process "git clone https://github.com/#{skel_repo} #{branch}"
...
When(/^I install the gems from "(.*)"$/) do |dir|
  run_process 'bundle install', dir, true
end

When(/^I install or check "(.*)" as "(.*)" from "(.*)" branch$/) do |repo, dir, branch|
  if ! Dir.exists?(dir)
    puts "Clone AutoGraders as '#{dir}'"
    run_process("git clone -b #{branch} https://github.com/#{repo} #{dir}" )
  else
